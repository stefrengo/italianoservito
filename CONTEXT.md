...
4. **Traduzioni PL/EN — COSTRUITE (ago 2026)**. Scope deciso con Stefano: NON tutto il sito, ma solo 4 pagine in entrambe le lingue — Home, Chi Sono, FAQ, Offerta Formativa (niente pagine `/percorsi/*`, niente `/contatti`, niente `/club-del-libro` in PL/EN). Stessi slug sotto `/pl/` e `/en/` di quelli italiani (`src/pages/pl/*.astro`, `src/pages/en/*.astro`). Workflow seguito: bozza di testo in `traduzioni-pl-en-bozza.md`, poi correzioni di Giada (file `.doc`, guardato solo il polacco), poi propagazione delle modifiche esplicite a IT/EN — comprese alcune correzioni al sito ITALIANO live (vedi commit relativo) — poi costruzione delle 8 pagine. Componenti condivisi resi multilingua con prop `locale`: `Navbar.astro` e `Footer.astro` (menu semplificato in PL/EN: Home, Chi Sono/O mnie/About me, FAQ + CTA, niente dropdown Percorsi né Contatti), `SignupForm.astro` (etichette tradotte), `PromiseCards.astro`/`Testimonials.astro` (accettano items via prop). `corsiStagione.js` ha campi `tipoPl`/`tipoEn`/`comeFunzionaPl`/`comeFunzionaEn` accanto a quelli italiani. Note operative: (a) le testimonianze di Kamila/Małgosia sono OMESSE in Home PL (servirebbe il testo originale polacco, non ancora recuperato) ma PRESENTI in EN (tradotte con cautela dall'italiano); (b) la sezione "I percorsi" della Home in PL/EN è un blocco unico verso Offerta Formativa, non la griglia CourseCards; (c) `/grazie` resta UNICA per le 3 lingue ma non è più statica (`prerender = false`): legge `?lang=pl|en` (passato da `/api/iscrizione`, che rileva la lingua dall'header Referer della pagina con il form) e mostra il blocco giusto, default italiano; (d) la mail di conferma iscrizione (`src/pages/api/iscrizione.js`) è localizzata allo stesso modo, la notifica interna a Giada resta in italiano con tag `[PL]`/`[EN]` nell'oggetto. Ancora aperto: un punto di dettaglio sulla FAQ (domanda 1, nota di Giada ambigua — vedi `traduzioni-pl-en-bozza.md`) e la revisione finale del polacco da parte di una madrelingua prima del lancio pubblico.
